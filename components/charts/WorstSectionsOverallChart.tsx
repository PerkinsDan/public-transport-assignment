...
interface WorstSectionsOverallChartProps {
  data: RouteSection[];
  limit?: number;
}
...
function sectionLabel(s: RouteSection) {
  return `${s.from_stop} → ${s.to_stop}`;
}

export default function WorstSectionsOverallChart({ data, limit = 10 }: WorstSectionsOverallChartProps) {
      ...
      <div className="relative p-4 sm:p-6">
        <div className="mb-4">
          <h3 className="text-lg font-bold tracking-tight text-slate-900 dark:text-white">
            Worst Sections Overall
          </h3>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Stop-to-stop segments with highest average delay across all routes
          </p>
        </div>
